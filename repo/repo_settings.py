...
def get_projects_from_manifests(args):
    projects = []

    with open(args.project_file) as xml_file:
        root = ElementTree.parse(xml_file).getroot()

        for project in root.findall("project"):
            name = project.get("name")
            revision = args.branch or project.get("revision") or root.find(
                "default").get("revision").split("/")[-1]
...
def main():
    ...
    print(gh_user)
    projects = get_projects_from_manifests(args)
